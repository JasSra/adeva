...
public class MessagesController : Controller
{
    ...
    public async Task<IActionResult> Inbox(int page = 1, int pageSize = 20, CancellationToken ct = default)
    {
        ...
        var items = await query.OrderByDescending(r => r.InternalMessage!.SentAtUtc)
            ...
            .Select(r => new ClientInboxItemVm
            {
    ...
    public async Task<IActionResult> View(Guid id, CancellationToken ct)
    {
        ...
        var vm = new ClientMessageDetailVm
        {
            Id = id,
            Title = rec.InternalMessage!.Title,
            Content = rec.InternalMessage.Content,
            Priority = rec.InternalMessage.Priority,
            Category = rec.InternalMessage.Category,
            SentAtUtc = rec.InternalMessage.SentAtUtc,
            ReadAtUtc = rec.ReadAtUtc,
            Status = rec.Status,
            RelatedEntityType = rec.InternalMessage.RelatedEntityType,
            RelatedEntityId = rec.InternalMessage.RelatedEntityId,
            IsSystemGenerated = rec.InternalMessage.IsSystemGenerated
        };
    ...
    public async Task<IActionResult> ToggleRead(Guid id, CancellationToken ct)
    {
        var userIdStr = User.FindFirstValue(ClaimTypes.NameIdentifier);
        if (!Guid.TryParse(userIdStr, out var userId)) return Unauthorized();
        var rec = await _db.InternalMessageRecipients.FirstOrDefaultAsync(r => r.InternalMessageId == id && r.UserId == userId, ct);
        if (rec == null) return NotFound();
        if (rec.Status == InternalMessageStatus.Unread) rec.MarkAsRead(); else rec.MarkAsUnread();
    ...
    public async Task<IActionResult> MarkReadAjax([FromBody] Guid id, CancellationToken ct)
    {
        ...
        await _db.SaveChangesAsync(ct);
        var unread = await _db.InternalMessageRecipients.CountAsync(r => r.UserId == userId && r.Status == InternalMessageStatus.Unread, ct);
    ...
    [HttpPost]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> Reply([FromForm] ClientReplyVm vm, CancellationToken ct)
    {
        ...
        // Clients cannot message others: only reply to originator
...
public class ClientInboxItemVm
{
...
public class ClientMessageDetailVm
{
    public Guid Id { get; set; }
    public string Title { get; set; } = string.Empty;
